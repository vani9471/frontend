...
const Timetables = () => {
    ...
    const fetchTimetable = async () => {
        ...
        setError('');
        try {
            const res = await axios.get(`${import.meta.env.VITE_API_URL || 'https://backend-1-x7ra.onrender.com'}/api/timetables?department=${selectedDept}&semester=${selectedSem}`, {
                headers: { Authorization: `Bearer ${JSON.parse(localStorage.getItem('user')).token}` }
            });
    ...
    const handleGenerate = async () => {
        setGenerating(true);
        try {
            await axios.post(`${import.meta.env.VITE_API_URL || 'https://backend-1-x7ra.onrender.com'}/api/timetables/generate`, {
